...
#2019-06-27: 使用h2代替websocket(隐藏效果待定),随机生成字符串作为v2ray的链接，不再固定为ws，增加nginx访问日志，建议偶尔上来看看是否有GFW探针访问的记录
...
*-----------------------------------------------------------------------
v2ray 一键安装脚本，自动安装v2ray, nginx, 自动申请证书，自动更新证书，自动生成http2+nginx+tls模式的服务端和客户端配置
使用方式：分别执行以下两行命令
[1] cd /usr/local && git clone https://github.com/abcfyk/impatriot.git && cd impatriot/v2ray
[2] bash h2_nginx_tls.sh -d 你的域名
  ...
  "inbounds": [
    {
      ...
      "streamSettings": {
        ...
        "security": "none"  //此处的inbounds是nginx转发的流量,无需再用tls加密
      },
      "tag": "in-0",
      "settings": {
        "clients": [
