...
if systemctl is-active --quiet xray; then
  echo -e "${GREEN}xray启动成功 (Xray service started successfully)${PLAIN}" 1>&2
else
  echo -e "${RED}xray启动失败，请检查日志 (Xray service failed to start, check logs)${PLAIN}" 1>&2
...
cat 1>&2 <<EOF
*-----------------------------------------------------------------------
INFO：XRAY 已经安装并配置完毕，你可以将客户端配置文件：/tmp/${PROXY_DOMAIN}.json 下载至本地导入到客户端使用
INFO：服务端XRAY版本为：${V_VERSION}，请注意客户端版本是否匹配
INFO：请打开浏览器访问 https://${PROXY_DOMAIN} ，若可以正常访问表示安装正常
*-----------------------------------------------------------------------
EOF
...
echo "--------------------------------------------------------------------"
echo "  VLESS 链接 (复制到剪贴板后在客户端内导入):"
echo "  ${VLESS_LINK}"
echo "--------------------------------------------------------------------"
echo "  使用客户端扫描以下二维码（截图后在客户端内导入也可）:"
echo ""
